            	...
            	if(!inGame) {
	                ...
	                case REQUESTED_GAME:
	                	...
	                	if(response == 0)  {
	                		//start game as host.
	                		inGame = true;
	                		window.changeToMultiplayer(in.getAddress(), 2000, true, null);
	                	} else {
	                		sendDatagram(new Message(Message.MessageType.END_GAME), server, in.getAddress(), port);
	                	}
	                	break;
	                case ACCEPTED_GAME:
	                	//start game as non host
	                	inGame = true;
                		window.changeToMultiplayer(in.getAddress(), 2000, false, ((UpdateBoard) m).getBoard());
	                	break;
	                case END_GAME:
	                	inGame = false;
	                	break;
